...
export class SetupComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // Validate the token exists (try a dry check by just showing the form — actual validation happens on submit)
    this.step.set('form');
  }
  ...
  onSubmit() {
    ...
    this.http.post<any>(`${this.baseUrl}/api/invitations/${this.token}/setup`, { password: this.password }).subscribe({
      ...
      error: (err) => {
        this.submitting.set(false);
        const errorMsg = err.error?.errors?.[0]?.message || 'Failed to set up account';
        const errorCode = err.error?.errors?.[0]?.code || '';

        if (errorCode === 'EXPIRED') {
          this.errorTitle.set('Invitation Expired');
          this.errorMessage.set('This invitation link has expired. Please ask your administrator to send a new one.');
          this.step.set('error');
        } else if (errorCode === 'USED') {
          this.errorTitle.set('Already Used');
          this.errorMessage.set('This invitation has already been used. You can sign in with your existing credentials.');
          this.step.set('error');
        } else if (errorCode === 'INVALID') {
          this.errorTitle.set('Invalid Link');
          this.errorMessage.set('This invitation link is invalid. Please check the link or ask for a new one.');
          this.step.set('error');
        } else {
